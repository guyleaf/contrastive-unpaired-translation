...
def get_transform(opt, params=None, grayscale=False, method=Image.Resampling.BICUBIC, convert=True):
    transform_list = []
    if grayscale:
...
def __random_zoom(img, target_width, crop_width, method=Image.Resampling.BICUBIC, factor=None):
    if factor is None:
